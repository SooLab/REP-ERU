...
def random_affine(img, mask, targets, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
    ...
    heightht = max(ht.shape[0], ht.shape[1]) + border * 2
    # Rotation and Scale
    R = np.eye(3)
    Rht = np.eye(3)
    ...
    Rht[:2] = cv2.getRotationMatrix2D(angle=a, center=(ht.shape[1] / 2, ht.shape[0] / 2), scale=s)
    # Translation
    T = np.eye(3)
    r1 = random.random()
    r2 = random.random()
    T[0, 2] = (r1 * 2 - 1) * translate[0] * img.shape[0] + border  # x translation (pixels)
    T[1, 2] = (r2 * 2 - 1) * translate[1] * img.shape[1] + border  # y translation (pixels)

    Tht = np.eye(3)
    Tht[0, 2] = (r1 * 2 - 1) * translate[0] * ht.shape[0] + border  # x translation (pixels)
    Tht[1, 2] = (r2 * 2 - 1) * translate[1] * ht.shape[1] + border  # y translation (pixels)
    # Shear
    S = np.eye(3)
