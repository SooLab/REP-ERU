...
class ReferDataset(data.Dataset):
    ...
    def pull_item(self, idx):
        ...
        ht = cv2.resize(ht, (256, 256))

        ptdir = self.im_dir.replace('images', 'depimg')
        ptfile = img_file #.replace('.jpg', '.jpeg')
        ...
        gestdir = self.im_dir.replace('images','gest')
        gestfile = img_file.replace('.jpg' , '.json')
        gest_path = osp.join(gestdir,gestfile)
        gest = json.load(open(gest_path))
        ...
        return img, pt, ht, phrase, bbox, gest
 #       return img, phrase, bbox, pt, ht
    ...
    def __getitem__(self, idx):
        img, pt, ht, phrase, bbox, gest = self.pull_item(idx)
        # phrase = phrase.decode("utf-8").encode().lower()

        center_point = gest['candidate']
        try:
            center_point = np.asarray(center_point)[:,0:2]
            if center_point[0,0] != 0:
                center_point = center_point [0,:]
            elif center_point[1,0] != 0:
                center_point = center_point [1,:]
            else :
                center_point = np.asarray([256,256])
            # mask = center_point!=0
            # print(center_point.shape)
            # center_point = center_point[mask]
            # print(center_point.shape)
            # center_point = center_point [0:2,:]
            # center_point = np.mean(center_point , axis = 0)
        except IndexError:
            center_point = np.asarray([256,256])
        ...
        if self.augment:
            ...
            if augment_flip and random.random() > 0.5:
                ...
                pt = cv2.flip(pt , 1 )
                ht = cv2.flip(ht , 1 )
                center_point[0] = w - center_point[0] - 1
            ...
            if augment_affine:
                img, _, bbox, M, center_point, pt, gt = random_affine(center_point, pt, img, None, bbox, \
        ...
        if self.testmode:
            return img, pt, ht, np.array(word_id, dtype=int), np.array(word_mask, dtype=int), \
                np.array(bbox, dtype=np.float32), np.array(ratio, dtype=np.float32), \
                np.array(dw, dtype=np.float32), np.array(dh, dtype=np.float32), self.images[idx][0], center_point, gt
        else:
            return img, pt, ht, np.array(word_id, dtype=int), np.array(word_mask, dtype=int), \
            np.array(bbox, dtype=np.float32), center_point, gt